...
api_id = "-----"
api_hash = '-----'
phone_number = '+' #use international format

group_name = -100         # relevant group ID (use telegram web to obtain the number eg: -100*******)
...
async def get_group_messages():
    ...
    async for message in client.iter_messages(group, limit=max_messages):
        if any(keyword.lower() in message.text.lower() for keyword in keywords):
            ...
            #the data will be stored in a json file in the directory
            value = {message.text: sentiment_score}
